...
# The run's durable read of itself, which outlives the run directory's working files and is what a
# finished run's coordinator is pointed at.
...
def report_path(run_dir, run):
    """The run's report, which lives beside the feature's tickets rather than in the run directory.

    Read off the run's own record of where it ran, so a run directory reached by another path than
    the one it recorded still names the one report it wrote.
    """
...
def adopt(args, repo, run_dir, table_path):
    ...
    if run_is_complete(records):
        ...
        snapshot(RUN_COMPLETE, pointer=str(report_path(run_dir, table.get("run") or {})))
        return 0
