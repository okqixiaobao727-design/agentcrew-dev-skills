...
class SessionCostTests(FakePaneTestCase):
    """The one `session-cost` line a Codex review leaves beside its `review` pair."""
    ...
    def test_a_review_records_what_its_thread_spent(self):
        ...
        self.assertEqual(entry["input_tokens"], ROUND_ONE_COUNTERS["input"])
        self.assertEqual(entry["output_tokens"], ROUND_ONE_COUNTERS["output"])
        self.assertEqual(entry["cache_read_tokens"], ROUND_ONE_COUNTERS["cache_read"])
        self.assertEqual(
            entry["cache_creation_tokens"], ROUND_ONE_COUNTERS["cache_creation"]
        )
        self.assertEqual(entry["total_tokens"], ROUND_ONE_USAGE["total_tokens"])
    ...
    def test_a_review_that_never_came_back_records_the_diagnosis(self):
        """The turn never finishes, so there is nothing to read — and the row still goes in."""
        self.write_rollout([turn_context(RESOLVED_MODEL), token_count(ROUND_ONE_USAGE)])
